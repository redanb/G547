...
#define CHIP_REG		0xD0
#define BMP085_CTRL_Reg			0xF4
#define BMP085_TEMP_MEAS		0x2E
#define BMP085_PRESSURE_MEAS	0x34

#define BMP085_CONVERSION_REG_MSB	0xF6
#define BMP085_CONVERSION_REG_LSB	0xF7
#define BMP085_CONVERSION_REG_XLSB	0xF8
#define BMP085_TEMP_CONVER_TIME	5

static int bmp085_probe(struct device *dev, struct regmap *regmap, int irq);
static int bmp085_remove(struct device *dev);
static int bmp085_detect(struct device *dev);
...
static s32 bmp085_update_raw_temperature(struct bmp085_data *data)
{
	...
	wait_for_completion_timeout(&data->done, 1 + msecs_to_jiffies(
					    BMP085_TEMP_CONVER_TIME));

	status = regmap_bulk_read(data->regmap, BMP085_CONVERSION_REG_MSB,
				 &tmp, sizeof(tmp));
	if (status < 0) {
		dev_err(data->dev,
...
int bmp085_detect(struct device *dev)
{
	...
	ret = regmap_read(data->regmap, CHIP_REG, &id);
	if (ret < 0)
		return ret;
